...
static void resize_event(gx_uvoxUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_uvoxUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_uvoxUI* ui = (gx_uvoxUI*)malloc(sizeof(gx_uvoxUI));
	...
	ui->controls[0] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},  { 50, 28, 61, 61}, false,"GAIN", KNOB, GAIN};
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},  { 120, 28, 61, 61}, false,"CLIPPING", KNOB, CLIPPING};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},  { 190, 28, 61, 61}, false,"BASS", KNOB, BASS};
	ui->controls[3] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},  { 260, 28, 61, 61}, false,"TREBLE", KNOB, TREBLE};
	ui->controls[4] = (gx_controller) {{ 0.0, 0.0, 0.0, 1.0, 1.0},   { 340, 28, 61, 61}, false,"REVERB", SWITCH, REVERB};
	ui->controls[5] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},  { 410, 28, 61, 61}, false,"-LEVEL", KNOB, REVERBLEVEL};
	ui->controls[6] = (gx_controller) {{ 0.25, 0.25, 0.0, 1.0, 0.01},{ 480, 28, 61, 61}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
	...
	ui->frame = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, 71, 81);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-140);
...
static void knob_expose(gx_uvoxUI *ui,gx_controller* knob) {
	...
	cairo_move_to (ui->crf, knobx1-extents.width/2, grow-4+extents.height);
	cairo_show_text(ui->crf, knob->label);
	cairo_new_path (ui->crf);
}

// draw the power switch (bypass)
static void bypass_expose(gx_uvoxUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_uvoxUI *ui, gx_controller* controller) {
...
static void _expose(gx_uvoxUI *ui) {
	static const char* plug_name = "GxUVox720k" ;
	...
	cairo_text_extents_t extents;
	cairo_set_source_rgba (ui->cr, 0.6, 0.6, 0.6,0.8);
	...
	cairo_move_to (ui->cr, ((double)(ui->width/2.0)/ui->rescale.x-(extents.width)/2.0),
	  (double)(ui->height)/ui->rescale.y-extents.height);
...
static void controller_expose(gx_uvoxUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
	...
	cairo_rectangle (ui->cr,(double)control->al.x * ui->rescale.x2,
	  ...
	  (double)control->al.width+10.0, (double)control->al.height+20.0);
	cairo_fill(ui->cr);
	cairo_stroke(ui->cr);
...
static void send_controller_event(gx_uvoxUI *ui, int controller) {
...
static void check_value_changed(gx_uvoxUI *ui, int i, float* value) {
...
static void check_is_active(gx_uvoxUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_uvoxUI *ui) {
...
static bool get_active_ctl_num(gx_uvoxUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_uvoxUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_uvoxUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_uvoxUI *ui, int i) {
...
static void button1_event(gx_uvoxUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_uvoxUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_uvoxUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_uvoxUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_uvoxUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_uvoxUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_uvoxUI *ui, bool set) {
...
static void event_handler(gx_uvoxUI *ui) {
	XEvent xev;
